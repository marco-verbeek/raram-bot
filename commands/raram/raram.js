...
/**
 * Displays an analysis of the player's last game || gameId.
 * @param {CommandoMessage} message - The message the command is being run for
 */
async function command_analyse(msg, gameId = 5399406298) {
  // axios request on endpoint /analyse/:gameId||:discordId
  // returns a message 'success'/'error' and the amount of total games in queue

  //const res = await axios.get('http://localhost:5000/api/analysis/only/' + gameId).data
  const res = require('../../data/example_game.json')
  const players = res["players"]
  let col1 = "", col2 = "", col3 = ""

  for(var i=0; i<5; i++){
    col1 += champ_icon_or_random(players[i]["champion"]) + " | " + players[i]["summonerName"] + "\n";
    col2 += players[i]["kills"] + "/" + players[i]["deaths"] + "/" + players[i]["assists"] + "\n";
    col3 += random_rank_icon() + " ("+ (players[i]["lpGain"] >= 0 ? "+" : "" ) + players[i]["lpGain"] + ")\n";
  }

  const embed = new MessageEmbed()
  .setAuthor("Here are your rARAM stats for your last played ARAM:", msg.author.displayAvatarURL())
  .setColor(0x00b3ff)
